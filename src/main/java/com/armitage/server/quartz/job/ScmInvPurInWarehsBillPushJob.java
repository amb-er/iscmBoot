...
public class ScmInvPurInWarehsBillPushJob implements Job{
	//private final static int excuteTimes = 9;
    private static Log log = LogFactory.getLog(ScmInvPurInWarehsBillPushJob.class);
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
        try {
            ...
            invPurInWarehsBillPush(arg0.getJobDetail().getKey().getName());
    ...
    public void invPurInWarehsBillPush(String jobName){
    	...
    	/*if(StringUtils.isNotBlank(jobName)){
    		...
    		if(params != null && params.length>=3){
    			...
    			log.info("执行采购入库单推送次数："+Long.parseLong(params[2])+"_"+count);
    			try {
					Thread.sleep(6000);
				...
				bJob.setJobName(ScmInvPurInWarehsBillPushJob.class.getSimpleName()+"_"+param.getControlUnitNo()+"_"+Long.parseLong(params[2]));
    			...
    			InvPurInWarehsBillPushTaskThread executeTaskThread = new InvPurInWarehsBillPushTaskThread();
				executeTaskThread.setPurInWarehsBillId(Long.parseLong(params[2]));
				executeTaskThread.setAdd(true);
				executeTaskThread.setParam(param);
				executeTaskThread.setName("Execute_Sync_Task_InvPurInWarehsBillPush_"+params[2]);
				executeTaskThread.start();
    		}
    	}*/
    }
