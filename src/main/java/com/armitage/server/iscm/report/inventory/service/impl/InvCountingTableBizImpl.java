...
public class InvCountingTableBizImpl extends BaseReportDataBizImpl implements BaseReportDataBiz{
	...
	protected void afterGetReportData(Report2 report,
		...
		if (list != null && list.size() > 0) {
	        for(int i=0;i<list.size();i++){
		        ...
		        orgUnitNo = orgBaseUnitBiz.selectbyOrgNo((String)keyMap.get("orgUnitNo"), param);
	            if (orgUnitNo != null) {
	                ((HashMap) obj).put("orgUnitNo", orgUnitNo.getOrgUnitName());
	            }
	            countingPerson = usrBiz.selectByCode((String)keyMap.get("countingPerson"), param);
		        if (countingPerson != null) {
		        	((HashMap) obj).put("creator", countingPerson.getName());
		        }
		        countingMonitorer = usrBiz.selectByCode((String)keyMap.get("checker"), param);
		        if (countingMonitorer != null) {
		        	((HashMap) obj).put("checker", countingMonitorer.getName());
		        }
			}
		}
	}
