...
public class ProcessInstanceHighlightsResource {
	  ...
	  ObjectNode responseJSON = objectMapper.createObjectNode();
		...
		try {
			...
			if(processInstance != null){
				...
				List<String> highLightedActivities = getHighLightedActivities(processDefinition, processInstanceId);
	 ...
	 * @return
	 */
	private List<String> getHighLightedActivities(ProcessDefinitionEntity processDefinition, String processInstanceId) {
		 ...
		 if(historicActivityInstances != null && !historicActivityInstances.isEmpty()){
			 for(HistoricActivityInstance hai : historicActivityInstances){
				if("userTask".equals(hai.getActivityType())){
					if(!historyTaskExist(hisActInstList, hai.getActivityId())){
		        		continue;
		        	}else{
    ...
    for (PvmTransition pvmTransition : pvmTransitionList) {
      ...
      if (destHisActInst != null) {
        if (isParallel) {
          ...
          if("userTask".equals(destHisActInst.getActivityType())){
        	if(!historyTaskExist(hisActInstList, destActId)){
        		continue;
        	}
          }
	...
	private boolean historyTaskExist(LinkedList<HistoricActivityInstance> hisActInstList, String actId){
		for (HistoricActivityInstance hisActInst : hisActInstList) {
			if (hisActInst.getActivityId().equals(actId)) {
				HistoricTaskInstance historyTask = historyService.createHistoricTaskInstanceQuery()
						.taskId(hisActInst.getTaskId()).singleResult();
				if (historyTask != null) {
					return true;
				}
			}
